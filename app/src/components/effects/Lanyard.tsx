...
const CARD_SCALE = 2.85;
const MOBILE_CARD_SCALE = 2.6;
...
function Lanyard({
  ...
  return (
    <div className="customer-lanyard">
      <Canvas
        camera={{ position, fov }}
...
type BandProps = Omit<LanyardProps, "position" | "gravity" | "fov"> & {
  maxSpeed?: number;
  minSpeed?: number;
  isMobile?: boolean;
};
...
function Band({
  ...
  const chainTarget = new THREE.Vector3();
  ...
  const anchorWorldX =
    anchorX === undefined
      ? isMobile
        ? 1.55
        : 2.6
      : ((anchorX / size.width) * 2 - 1) * (viewport.width / 2);
  ...
  useFrame((state, delta) => {
    if (dragged) {
      ...
      const target = vec.sub(dragged);
      const anchor = fixed.current.translation();
      chainTarget.set(target.x, target.y + 1.8, target.z);

      [j1, j2, j3].forEach((ref, index) => {
        const progress = (index + 1) / 3;
        chainPoint.set(
          anchor.x + (chainTarget.x - anchor.x) * progress,
          anchor.y + (chainTarget.y - anchor.y) * progress,
          anchor.z + (chainTarget.z - anchor.z) * progress,
        );
        ref.current?.setNextKinematicTranslation(chainPoint);
      });
  ...
  return (
    <>
      <group position={[anchorWorldX, viewport.height / 2 - 0.08, 0]}>
        ...
        >
          ...
          <group
            scale={isMobile ? MOBILE_CARD_SCALE : CARD_SCALE}
            position={[0, -1.5, -0.05]}
